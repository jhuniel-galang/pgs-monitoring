...
class Task extends DatabaseModel {
    ...
    // Get all tasks (UPDATED with year)
    public function getAllTasks() {
        $query = "SELECT t.*, 
                  ...
                  GROUP BY t.task_id
                  ORDER BY t.year DESC, t.functional_division, t.priority DESC, t.created_at DESC";
    ...
    // For encoders - only get tasks from their division (UPDATED with year)
    public function getTasksByDivisionForEncoder($division, $user_id = null) {
        $query = "SELECT t.*, 
                  ...
                  GROUP BY t.task_id
                  ORDER BY t.year DESC, t.priority DESC, t.created_at DESC";
    ...
    public function getTasksByDivision($division) {
        $query = "SELECT t.*, u.unit_name, u.person_in_charge, p.project_name,
                  (SELECT percentage FROM tbl_status WHERE task_id = t.task_id ORDER BY created_at DESC LIMIT 1) as current_percentage,
                  (SELECT remarks FROM tbl_status WHERE task_id = t.task_id ORDER BY created_at DESC LIMIT 1) as latest_remarks
                  FROM " . $this->table . " t
    ...
    public function getTasksWithFilters($filters = [], $limit = 10, $offset = 0) {
        ...
        $query .= " GROUP BY t.task_id ORDER BY t.year DESC, t.created_at DESC";
    ...
    public function getTotalTaskCount($filters = []) {
        ...
        if(!empty($filters['division'])) {
            $whereConditions[] = "t.functional_division = :division";
            $params[':division'] = $filters['division'];
        }
        ...
        if(isset($filters['role']) && $filters['role'] == 'encoder' && !empty($filters['user_division'])) {
            $whereConditions[] = "t.functional_division = :user_division";
    ...
    // Get tasks for a specific project (UPDATED with year)
    ...
    try {
        ...
        $query = "INSERT INTO tbl_task 
                  (task_details, project_id, functional_division, year,
    ...
    // Get task with its units (UPDATED with year)
